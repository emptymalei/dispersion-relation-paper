...
More realistic models of supernova explosions involves continuous zenith angle ELN spectra. In this section we analyze dispersion relation and instabilities for box-shaped spectra as well as spectrum from Garching group, both of which are assumed to be axial symmetric.

\subsection{\label{sec-boxspectrum}Box Spectra}

As the first example of continuous emission, we calculate the dispersion relation and linear stability analysis for a box spectrum.
...
\begin{figure}
   ...
   \endminipage\hfill
   \caption{Dispersion relation and linear stability analysis for box spectrum. The box spectrum is $-0.1$ within range $u\in [-1,-0.3)$ and $1$ within range $u\in [-0.3,1]$ as shown in the left panel. The right panel shows that the dispersion relation and the linear stability analysis. Dashed gray lines are lines of $\omega= \pm k$ which sets the boundaries of the forbidden region for dispersion relation.
    }
   \label{fig-box-c1}
\end{figure}
