...
\author{Lei Ma}
\email{leima@unm.edu}
%  \altaffiliation[Also at ]{Physics Department, University of New Mexico.}%Lines break automatically or can be forced with \\
\author{Huaiyu Duan}%
 \email{duan@unm.edu}
\affiliation{%
...
Dispersion relation has been proven to be related to collective neutrino oscillations. The notion that instabilities is related to gaps in neutrino oscillations has been shown to be true in several examples. However, we find that it is not generally true for many situations. We propose that those gaps indicating instabilities should be defined as gaps between dispersion relations and the axes. Meanwhile, instabilities are not always shown as gaps in dispersion relations.
...
\begin{align}
&\left(4\operatorname{Re}(k) - \mathcal P \int \frac{G(u)}{u} \mathrm d u + U_1 \right)^2 \nonumber \\
&- \left( \operatorname{Sign}(\omega \operatorname{Im}(k) )\pi G(0) +4 \operatorname{Im}(k) \right)^2  \nonumber \\
...
The first author would like to thank S. Abbar and J. Martin for their help during this research. This research is supported by DOE EPSCoR grant \#DE-SC0008142.
